...
class Saver():
    ...
    def save(self, new_val, dict_to_save, logger):
        ...
        if self._do_save(new_val):
            ...
            #logger.info("save dict")
            torch.save(dict_to_save, '{}/best_checkpoint_Version{}_IoU{}.pth.tar'.format(self.ckpt_dir, self.version, self.best_val))
            #logger.info("gespeichert")
            return True
        else:
            torch.save(dict_to_save, '{}/worst_checkpoint_Version{}_IoU{}.pth.tar'.format(self.ckpt_dir, self.version, new_val))
        return False
