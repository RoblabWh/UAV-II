...
def plane_intersection(LP0, LP1, n, d):
    ...
    if abs(D) < 0.01:
        if N == 0:
            return 'in plane' #line in plane
        else:
            return 'is parallel' #line parallel to plane
    sI = N / D
    #if sI < 0 or sI > 1:
    #    return 'not in front'
...
binCurvImg = binCurvImg(curvImg, threshold=0.008)
    ...
    for p in segment:
        ...
        for nx in neighborhoodWndX:
            for ny in neighborhoodWndY:
                ...
                #print(intersection)
                if intersection not in ['in plane', 'is parallel', 'not in front']:
