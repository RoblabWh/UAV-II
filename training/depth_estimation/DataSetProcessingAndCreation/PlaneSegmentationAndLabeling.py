...
This script takes Pointclouds in our .npy-Format and automatically segments Planes from them.
The Pointclouds are created externally from this script by processing Depth images from a Microsoft Kinect.

The segmentation of planes results in an .npy-file with the 3d-coordinates of points related to planes and a binary
image which shows at which pixel in the original depth image a point is seen which fits to such a plane.

The purpose of this plane segmentation is to create a labeled dataset for the training of NNs for the depth estimation
in corridors. The original depth image shall act as ground truth and the binary mask shall be an indicator for places in
the image which are of special interest.

Special requirements for this segmentation script were:
*High Reliabilty
*High Performance

It was assumed that the input dataset is well suited to find walls of corridors so that there is no need to specifially
find situations in where our algorithm might fail.
...
expectedRelPointDistanceUnderNoise = 0.1 #for mean filter - what is the assumed distance of a point to itself due to noise?
...
def getSimPCLFromObject(resx, resy, objectType='plane'):
    '''

    This functions generated a PCL from different model types such as plances of hemispheres.
    The purpose of this function is to provide a simulated PCL for testing 3D-algorithms.
    The PCLs can be noisy or deterministic depending on the objectType-param passed to this function.

    :param resx: Number of Pixels in the horizontal direction (x-direction)
    :param resy: Number of Pixels in the vertical direction (y-direction)
    :param objectType: Must be one of the following (pass as string): plane, noisyPlane, hemisphere, NoisyHemisphere

    :return: Deterministic or noisy Poincloud
    '''
...
def binaCurvImg(curvImg, threshold=0.03):
    ...
    :param curvImg: The input image representing the curvature (change of Normals to be exact) per pixel
    :param threshold: Threshold for binarisation. Notice that the curvature here has no means in term of physicy, that is,
    we only calculate a measure with a similar meaning than that of the curvature. As such, a reasonable Threshold must
    be found empircally.
...
def segmentBinCurvImg(binCurvImg, minSegmentSizeRel=0.025):
    '''
    This function takes a binarisied Image of the "curvature" in 3D-space and finds and returns the 2D-segments.
    The algorithms used here for the segmentation is a classical non optimized 2D-Region Growing algorithm.

    :param binCurvImg: Binary Image of the "curvature" in 3D- space
    :param minSegmentSizeRel: Pass this parameter to determine the minimum size of a segment relative to the image size.
    If
    :return:
    '''
    segments = []
...
def getThreeRandomPoints(points):
    '''
    This function return generates three random indices for the array of points passed by points
    and returns three points selected by these indeces.

    :param points: Array of 3D-points in a PCL
    :return: Three 3D-points randomly selected from the array points
    '''
...
def ransacPlane(segment, localNormals):
    '''
    This function implements a standard RANSAC-algorithm for finding a consensus set of 3D-points which fit to a plane
    under the assumptions of noise.

    Important: The parametrisation of this RANSAC is specifially optimized in the context of this script.
    That is, it depends on the quality of results from the curvature based presegmentation followed
    by the REGION-GROWING-based segmentation.

    In the context of this script we only need a small amount of iterations which makes this RANSAC
    operating extremely fast on our datasets.

    :param segment: Array of 3D-points representing a segment of the 3D-PCL which is a candidate for a consensus set
    for planes.
    In this script the candidates are found by curvature filtering while the segments are provided by Region Growing.
    :param localNormals: Normal vectors per 3D point found by previous curvature operations.
    :return: Tupel:
    First element: List of Consensus sets, whereby the consensus sets are Lists of 3D-Points.
    Second element: Tupel of the normal vector (3D) and the corresponding plane distance from the scene center
    for the absoulte best consensus set.
    '''
    filteredSegment = []
...
def filterOutlierSegments(segments, localNormals):
    '''
    This function wraps the RANSAC-algorithm to find an optimal consensus set for 3D points best representing a plane
    in the given segments of 3D-points (which basically are pretty good canditates for consensus sets in our script)

    :param segments: Array of array of 3D-points representing segments and potential planes of the 3D-PCL
    :param localNormals: Normal vectors per 3D point found by previous curvature operations.
    :return: Tupel:
    First element: List of 3D-PCLs representing planes in the given segments.
    Second element: List of corresponding plane models each of which consists of a tupel of:
    Plane model with the normal vector (3D) and the corresponding plane distance from the scene center
    for the absoulte best consensus set.
    '''
    filteredSegments = []
...
def plane_intersection(LP0, LP1, n, d):
    '''
    PROTOTYPE
    This function implements a special plane intersection algorithm found at:
...
def checkDistanceToPlane(p, plane,threshold=0.02):
    '''
    This functions takes a 3D-point an a plane model and checks whether the distance of the given point to the given
    plane is below a specific threshold or not.

    :param p: Numpy array of an 3D-point.
    :param plane: Plane model defined by a tupel of: Normal vector, Distance of the plane to a scene center.
...
def labelPlanes(pointcloudPath, outputPathWithFileBasename):
    '''
    This function takes a path fo a PCL saved in the *.npy data format and produced extenillay from data from the
    Depth sensoring device Microsoft Kinect v1, automatically detects Planes the PCL and writes the data to the
    specified outputPath. See Docs for more information about the algorithm structure and purpose.

    :param pointcloudPath: Path to the *.npy-File holding the PCL generated from a depth image from the Microsoft
    Kinect v1.
    :param outputPathWithFileBasename: Output path where the output data of the plane segmentation shall be written to.
    Primarily two kinds of data are to be saved: First a binaray images in the *.png format representing pixels of
    the Kinects aligned depth image which correspond to planes in the PCL. And secondly the Plane-Segments are stored
    in 3D as a poincloud in the same .npy-format as the input PCL.

    Depending on your adjustements of this function additional intermediate data can be stored such as curvature images
    and others. Be aware that the intermediate data is not necessary for our training purpose but for testing, optimizing
    and debugging and that the size of your dataset incereases drastically if you decide to save all intermediate data.

    :return: Number of planes found in the PCL specified by the given Path
    '''
