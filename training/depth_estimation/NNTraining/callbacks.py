...
from utils_ import DepthNorm, predict, evaluate
...
def make_image(tensor):
    ...
    output.close()
    return tf.compat.v1.Summary.Image(height=height, width=width, colorspace=channel, encoded_image_string=image_string)
        ...
        def on_epoch_end(self, epoch, logs=None):            
            if not test_set == None:
                ...
                for i in range(self.num_samples):
                    ...
                    x_train, y_train = x_train[0], np.clip(DepthNorm(y_train[0], maxDepth=65536), minDepth, maxDepth) / maxDepth
                    x_test, y_test = x_test[0], np.clip(DepthNorm(y_test[0], maxDepth=65536), minDepth, maxDepth) / maxDepth
                ...
                self.writer.add_summary(tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag='Train', image=make_image(255 * np.hstack(train_samples)))]), epoch)
                self.writer.add_summary(tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag='Test', image=make_image(255 * np.hstack(test_samples)))]), epoch)
