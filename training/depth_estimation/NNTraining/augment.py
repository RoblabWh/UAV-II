...
class BasicPolicy(object):
    ...
    def __call__(self, img, depth, planeSegments, ext_mirror_ration=None, ext_color_change_ratio=None):
        ...
        if random.uniform(0, 1) >= ext_color_change_ratio or self.color_change_ratio:
            policy_idx = 0

        img = img[...,list(self.indices[policy_idx])]

        # 2) Mirror image
        if random.uniform(0, 1) <= ext_mirror_ration or self.mirror_ratio:
            img = img[...,::-1,:]
            depth = depth[...,::-1]
            planeSegments = planeSegments[...,::-1]
        ...
        return img, depth, planeSegments
