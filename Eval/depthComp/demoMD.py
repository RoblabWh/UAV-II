...
class Window(QtWidgets.QWidget):
    ...
    def loadDepthImageFile(self):
        ...
        fileRgbName_disp = fileDepth.split('.')[0]
        fileRgbName_dispIndex = fileRgbName_disp.index('_disp')
        fileRgbName = fileRgbName_disp[:fileRgbName_dispIndex] + '.png'
        
        imgSci = imageio.imread(fileRgbName)
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.loadedDepth = np.load('0011_disp.npy')
        #intermDepth = [[col if col < 5 else 5 for col in row] for row in self.loadedDepth]
        #print(np.max(self.loadedDepth), np.min(self.loadedDepth))
        self.imgSci = imageio.imread('0011.png')

        self.rgb = self.imgSci[self.imgSci.shape[0]//2+1:]
