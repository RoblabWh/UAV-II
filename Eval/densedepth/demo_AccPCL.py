    ...
    def updateCloud(self):
        ...
        if self.model:
            ...
            with graph.as_default():
                ...
                # print("depth from inferencen: min-{}, max-{}; depth as used: min-{}, max-{}".format(np.min(depth1), np.max(depth1), np.min(depth), np.max(depth)))
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def updateRGBD(self):
        ...
        # Reshape
        points1 = self.posFromDepth(self.depth.copy())
        # pointsShuffled = np.random.shuffle(points1)
        np.random.shuffle(points1)
        points = points1[::4]

        print('points1: {}\npoints: {}'.format(points1.shape, points.shape))

        '''self.trans = 0.015
        self.pcl = np.array([])
        self.cloudCounter = 0'''
        points += (self.cloudCounter * np.array((0, 0, self.trans)))
        self.cloudCounter += 1

        self.pcl = np.concatenate((self.pcl, points))

        colors = resize(self.rgb, (height // 2, width // 2)).reshape(((height // 2) * (width // 2), 3))
        # colorsBlack = np.zeros_like(colors)
        colorsBlack = np.zeros((self.pcl.shape[0], 4), dtype=np.float32)

        # Flatten and convert to float32
        self.pos = self.pcl.astype('float32')
        # self.col = colors.reshape(height * width, 3).astype('float32')
        self.col = colorsBlack
        ...
        print('points1: {}\npoints: {}\nself.pcl: {}\nself.col: {}'.format(points1.shape, points.shape, self.pcl.shape,
                                                                           self.col.shape))
