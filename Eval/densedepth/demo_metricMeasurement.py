...
class Window(QtWidgets.QWidget):
    ...
    def loadImageFile(self):
        ...
        '''
        # labor horizontal (floor width) points (pixel) 120, 163 ---- 207, 163
        py1 = 163
        py2 = 163
        px1 = 120
        px2 = 205 #163
        ...
        #rgb8 = qimage_to_np(img)
        #print(rgb8.shape, rgb8[py1][px1].shape,rgb8[py1][px1][0])
        #rgb8[py1][px1] = (rgb8[py1][px1][0], 255, 0, 0)
        #rgb8[py2][px2] = (rgb8[py2][px2][0], 255, 0, 0)

        #rgb8[py3][px3] = (rgb8[py3][px3][0], 255, 0, 0)
        #rgb8[py4][px4] = (rgb8[py4][px4][0], 255, 0, 0)
        #print(rgb8[20][20][0], rgb8[20][20][1], rgb8[20][20][2], rgb8[20][20][3])

        img.setPixelColor(QtCore.QPoint(px1, py1), QtGui.QColor(255,0,0,0))
        img.setPixelColor(QtCore.QPoint(px2, py2), QtGui.QColor(255, 0, 0,0))
        img2 = img.scaledToHeight(rgb_height)
        '''

        # 150/2, 400/2 ----- 500/2, 400/2
        # Keller horizontal
        ...
        xstart = 0
        if img2.width() > rgb_width: xstart = (img2.width() - rgb_width) // 2
        img = img2.copy(xstart, 0, xstart + rgb_width, rgb_height)
        self.inputViewer.setPixmap(QtGui.QPixmap.fromImage(img2))
    ...
    def updateCloud(self):
        ...
        if self.model:
            ...
            with graph.as_default():
                depth1 = self.model.predict(np.expand_dims(self.glWidget.rgb, axis=0))
                depth = (1000 / depth1) / 1000
                # depth = (1000 / self.model.predict( np.expand_dims(self.glWidget.rgb, axis=0)  )) / 1000
                print("depth from inferencen: min-{}, max-{}; depth as used: min-{}, max-{}".format(np.min(depth1),
                                                                                                    np.max(depth1),
                                                                                                    np.min(depth),
                                                                                                    np.max(depth)))
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def posFromDepth(self, depth):
        ...
        # horizontal indezes
        index1 = px1 + (rgb_width // 2) * py1
        index2 = px2 + (rgb_width // 2) * py2

        # index1 = px1 + rgb_width * py1
        # index2 = px2 + rgb_width * py2

        '''
        # labor vertical indezes
        index3 = px3 + (rgb_width // 2) * py3
        index4 = px4 + (rgb_width // 2) * py4
        ...
        points = np.dstack((self.xx * z, self.yy * z, z)).reshape((length, 3))

        # labor horizontal (floor width)
        pLaborHor1 = (points[index1][0], points[index1][1], points[index1][2])
        pLaborHor2 = (points[index2][0], points[index2][1], points[index2][2])
        print("\npLaborHor1:{}\npLaborHor2:{}\n".format(pLaborHor1, pLaborHor2))
        delta_p1Hor = np.linalg.norm(np.array(pLaborHor2) - np.array(pLaborHor1))
        print("pLaborHorLength: {}".format(delta_p1Hor))

        # labor vertical (floor heigth)
        '''
        pLaborVer1 = (points[index3][0], points[index3][1], points[index3][2])
        pLaborVer2 = (points[index4][0], points[index4][1], points[index4][2])
        print("\npLaborVer1:{}\npLaborVer2:{}\n".format(pLaborVer1, pLaborVer2))
        delta_pVer1 = np.linalg.norm(np.array(pLaborVer2) - np.array(pLaborVer1))
        print("pLaborHorLength: {}".format(delta_pVer1))
        '''
