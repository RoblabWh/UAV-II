...
def qimage_to_np(img):
    ...
    print(img.size())
    print(np.array(img.constBits()).shape)
...
class TfEngine(object):
  def __init__(self, graph):
    ...
    config = tf.ConfigProto(gpu_options=
      tf.GPUOptions(per_process_gpu_memory_fraction=0.7,allow_growth=True))
...
class TftrtEngine(TfEngine):
  ...
  def infer(self, x):
    ...
    y = np.empty((num_tests, 240, 320, 1), np.float32)
...
class Window(QtWidgets.QWidget):
    ...
    def loadModel(self):        
        ...
        tic()  
        model = load_model()
        img = np.expand_dims(self.glWidget.rgb, axis=0)
        img = img.reshape((img.shape[1], img.shape[2], img.shape[3]))
        img_shape = img.shape

        frozenModel = FrozenGraph(model, img_shape)
        self.model= TftrtEngine(frozenModel, 1, 'FP32')
        print('Model loaded.')
        toc()
        self.updateCloud()
    ...
    def updateCloud(self):
        ...
        if self.model: 
            with graph.as_default():
                ...
                img_shape = img.shape
                t0 = time.time()
                depth = (1000 / self.model.infer( img  )) / 1000
